...
RESULTS_CSV = "perceptual_study_results.csv"
GROUPING_CSV = "participant_groupings.csv"

# Initialize the results CSVs if they don't exist
if not os.path.exists(RESULTS_CSV):
    pd.DataFrame(columns=[
        "Participant_ID", "Track_Name", "True_State", "Clarity", 
        "Stress_Urgency", "Reassurance", "Intuitive_Mapping", "Qualitative_Comments"
    ]).to_csv(RESULTS_CSV, index=False)

if not os.path.exists(GROUPING_CSV):
    pd.DataFrame(columns=["Participant_ID", "Guessed_Apnea_Tracks", "Guessed_Normal_Tracks"]).to_csv(GROUPING_CSV, index=False)
...
def survey_block(track_info, track_index):
    ...
    try:
        ...
        # Likert Scales
        clarity = st.slider(f"Clarity: How clearly does this represent a distinct health state?", 1, 5, 3, key=f"c_{track_index}")
        stress = st.slider(f"Stress/Urgency: What level of urgency does this induce?", 1, 5, 3, key=f"s_{track_index}")
        reassure = st.slider(f"Reassurance: Does the background rhythm provide a sense of safety?", 1, 5, 3, key=f"r_{track_index}")
        mapping = st.slider(f"Intuitive Mapping: How naturally does this sound map to a medical event?", 1, 5, 3, key=f"m_{track_index}")
        ...
        if st.button(f"Submit Ratings for {display_name}", key=f"btn_{track_index}"):
            ...
            new_data.to_csv(RESULTS_CSV, mode='a', header=False, index=False)
            st.success(f"Response for {display_name} saved successfully!")
...
if st.button("Submit Final Classification"):
    if len(guessed_apnea) == 3:
        ...
        grouping_data.to_csv(GROUPING_CSV, mode='a', header=False, index=False)
